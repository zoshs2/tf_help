...
Color = 'RGB' # or observational time : {10h, 100h}, otherwise, 'RGB' denotes '1000h' for observational times.
Noise = 'NN' # WG_{hours}h: WHITE_GAUSSIAN_NOISE , UNI : UNIFORM_NOISE, NN : Without Nowise
Smoothing = 'BS2_BW1'
...
n_splits = 10
epochs = 500 # 상당히 좋은 200
random_state = 2 # 상당히 좋은 2
stp=5
lr = 1e-3 # 상당히 좋은 1e-3
batch_size = 128 # 상당히 좋은 8
...
for idx, (trainIDX, valIDX) in enumerate(skf.split(n_sample, n_sample)):
    ...
    callbacks_list = [checkpoint, tb_hist] # ,early_stopping] : 테스트할때는 생략하고 epoch풀로 돌리기.
...
os.system("tar -cvf {}_{}_{}_batsize_test{}_Eps{}.tar {}_{}_{}".format(Smoothing, Noise, Color, batch_size, epochs, Smoothing, Noise, Color))
