...
for idx, (trainIDX, valIDX) in enumerate(skf.split(n_sample, n_sample)):
    ...
    model = Sequential()
    model.add(Conv2D(64, (5,5), padding='valid', input_shape=(200,200,3)))
    ...
    hist = model.fit(train_X, train_Y, epochs=200, batch_size=batch_size, validation_data=(val_X, val_Y), callbacks=callbacks_list)
